...
def Power_norm_complex(z, P = 1): 
    ...
    z_out = torch.zeros(batch_size, z_dim).cpu()
...
def AWGN_channel(x, snr, P = 1):  
    ...
    gamma = 10 ** (snr / 10.0)
    noise = torch.sqrt(P/gamma)*torch.randn(batch_size, length).cpu()
    y = x+noise
    return y
...
def AWGN_complex(x, snr, Ps = 1):  
    batch_size, length = x.shape
    gamma = 10 ** (snr / 10.0)
    n_I = torch.sqrt(Ps/gamma)*torch.randn(batch_size, length).cpu()
    n_R = torch.sqrt(Ps/gamma)*torch.randn(batch_size, length).cpu()
...
def Fading_channel(x, snr, P = 1):
    ...
    h_I = torch.randn(batch_size, K).cpu()
    h_R = torch.randn(batch_size, K).cpu() 
    h_com = torch.complex(h_I, h_R)  
    x_com = torch.complex(x[:, 0:feature_length:2], x[:, 1:feature_length:2])
    y_com = h_com*x_com
    
    n_I = torch.sqrt(P/gamma)*torch.randn(batch_size, K).cpu()
    n_R = torch.sqrt(P/gamma)*torch.randn(batch_size, K).cpu()
    noise = torch.complex(n_I, n_R)
    
    y_add = y_com + noise
    y = y_add/h_com
    
    y_out = torch.zeros(batch_size, feature_length).cpu()
...
def Power_norm_VLC(z, cr, P = 1):
    ...
    z_power = torch.sqrt(torch.sum(z**2, 1))
    z_M = z_power.repeat(z_dim, 1).cpu()
...
def AWGN_channel_VLC(x, snr, cr, P = 1):  
    batch_size, length = x.shape
    gamma = 10 ** (snr / 10.0)
    mask = mask_gen(length, cr).cpu()
    noise = torch.sqrt(P/gamma)*torch.randn(1, length).cpu()
    noise = noise*mask
    y = x+noise
    return y
