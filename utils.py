...
def Load_cifar10_data():
    ...
    print(f"Original x_train shape: {x_train.shape}")  # Check shape before transposition
    print(f"Original x_test shape: {x_test.shape}") 
    # from keras.datasets import cifar10
    # (x_train, y_train_), (x_test, y_test_) = cifar10.load_data()        
    #x_train = np.transpose(x_train, (0, 3, 1, 2))
    #x_test = np.transpose(x_test, (0, 3, 1, 2))
    #print(f"Transformed x_train shape: {x_train.shape}")  # Check shape after transposition
...
def Load_cifar100_data():
    ...
    print(f"x_train shape: {x_train.shape}, x_test shape: {x_test.shape}")  # Debugging line
    x_train = x_train.astype('float32') / 255
    x_test = x_test.astype('float32') / 255
    return x_train, x_test
